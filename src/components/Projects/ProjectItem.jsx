...
export default function ProjectItem() {
  ...
  useEffect(() => {
    setProject(projectsData.find((project) => project.id === Number(id)));
    const fetchData = async () => {
      try {
        console.log("ESTADO PROJECT --> ", project);
        const projectInfo = projectsData.find((p) => p.id === Number(id));
        await new Promise((res) => setTimeout(res, 0));
        setProject(projectInfo);
        console.log("ESTADO PROJECT POST SET --> ", project);
      } catch (error) {
  ...
  return (
    <div className="w-full cursor-default font-['Montserrat'] transition-colors duration-75 ease-in-out bg-white dark:bg-[#2a2a2a]">
      ...
      <div className="max-w-7xl w-full flex flex-col mx-auto pb-10 px-5 xs:px-10 sm:px-16 md:px-20 text-[#2a2a2a] dark:text-[#8a8a8a]">
        {
          !project ? <div className="w-full h-screen bg-green-500 flex items-center justify-center"><h2 className="text-5xl font-extrabold">CARGADO...</h2></div> :
          <>
            <section className="flex md:flex-row animate-fadeIn">
            ...
            <p className="text-base font-medium mb-7 pr-5 animate-fadeIn md:text-lg md:pr-10 lg:pr-32">{project.subtitle}</p>

            <section className={`flex flex-col items-center animate-fadeIn ${project.phone ? "lg:h-[600px] lg:flex lg:flex-row lg:items-stretch" : "xl:h-[380px] xl:flex xl:flex-row xl:items-stretch"}`}>
