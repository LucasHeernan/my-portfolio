...
export function useProject({ work }) {
  ...
  useEffect(() => {
    isInView && card ? animate([[ work, { transform: "translateY(0px)", opacity: 1 }, { delay: 0.2, ease: [0.17, 0.55, 0.55, 1], duration: 1 } ]]) :
    !isInView && !card ? animate([[ work, { transform: "translateY(25px)", opacity: 0 } ]]) :
    setCard(true);
