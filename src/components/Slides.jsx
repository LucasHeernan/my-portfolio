...
export default function Slides({ title, description, techs, image, images, repository, phone, curr }) {
  ...
  return (
    ...
    >
      <div className="absolute flex inset-y-4 pb-4 md:inset-y-5 md:px-3 md:pb-3 lg:inset-x-16">
        ...
        <div className="w-full h-full flex z-10 rounded-sm overflow-hidden md:shadow-lg md:shadow-black">
          ...
          >
            ...
            <div className="hidden absolute bottom-5 left-[calc(50%-45px)] z-10 group-hover:block group-hover:animate-bounceInUp md:group-hover:hidden">
              <div
          ...
          <section className={`hidden md:flex w-[55%] h-full flex-col justify-evenly py-10 ${!phone ? "md:pl-5" : ""} lg:pl-8 lg:py-5 bg-gradient-to-r from-indigo-300 via-indigo-200 to-indigo-100`}>
            <h3 className="tracking-wide md:text-3xl md:font-semibold lg:text-4xl">{title}</h3>
            <p className="text-base lg:text-lg font-normal pr-4">{description}</p>
            <div className="w-full flex">
              <div
                className="inline-block p-1 bg-[#313131] text-[#999] border-solid border-t-[1px] border-r-2 border-b-[3px] border-l-[1px] border-[#222] rounded-sm uppercase font-semibold text-sm shadow-md shadow-[rgba(0,0,0,.25)] mr-2 relative overflow-hidden box-border cursor-pointer
                before:bg-[#515151] before:top-0 before:left-0 before:w-0 before:h-1/2 hover:before:w-full before:block before:absolute before:z-10 before:duration-200 before:ease-in-out
                after:bg-[#3f3f3f] after:bottom-0 after:right-0 after:w-0 after:h-1/2 hover:after:w-full after:block after:absolute after:z-10 after:transition-all after:duration-200 after:ease-in-out after:delay-100"
              >
                <span className="relative z-20">ver más</span>
