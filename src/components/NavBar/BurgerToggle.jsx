...
export default function BurgerToggle({ isOpen, handleMenu }) {
  ...
  return (
    <button className="absolute left-1 xs:left-3 -top-[4px] p-2" onClick={handleMenu}>
      <i className={`block w-8 h-[3px] my-1 rounded transition-all ease-linear duration-500 ${isOpen ? "bg-[#ebebeb] dark:bg-[#ebebeb] rotate-[135deg] translate-y-7 translate-x-3 scale-125" : "-translate-y-0.5 bg-[#2a2a2a] dark:bg-[#fff]"}`}></i>
      ...
      <i className={`block w-8 h-[3px] my-1 rounded transition-all ease-linear duration-500 ${isOpen ? "bg-[#ebebeb] dark:bg-[#ebebeb] rotate-[225deg] translate-y-[14px] translate-x-3 scale-125" : "translate-y-0.5 bg-[#5b656d] dark:bg-[#9a9a9a]"}`}></i>
